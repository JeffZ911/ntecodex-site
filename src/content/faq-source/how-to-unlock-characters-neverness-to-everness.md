---
title: "How to Unlock Characters in Neverness to Everness: Full Guide"
slug: how-to-unlock-characters-neverness-to-everness
article_type: faq
qa_score: 4.7
word_count: 1838
published_at: "2026-05-24T22:51:19.125583+00:00"
published_url: /faq
sources: []
game: unknown
hero_image: /img/how-to-unlock-characters-neverness-to-everness/hero.webp
inline_images:
  - /img/how-to-unlock-characters-neverness-to-everness/inline-1.webp
  - /img/how-to-unlock-characters-neverness-to-everness/inline-2.webp
  - /img/how-to-unlock-characters-neverness-to-everness/inline-3.webp
  - /img/how-to-unlock-characters-neverness-to-everness/inline-4.webp
inline_image_sections:
  - "Question Restated"
  - "Direct Answer"
  - "Detailed Explanation"
  - "Related"
---

Starting a journey in Hethereau requires a strong team of Espers to handle the anomalies lurking in the city streets. This guide breaks down every method to expand your roster, from the initial story recruits to the unique board-game gacha system.

# How to Unlock Characters in Neverness to Everness
> ⚠️ **Notice:** Auto-generated content. May contain approximations or minor inaccuracies in supporting details. Help us improve via the comment section below. _Last reviewed: 2026-05-24._


The launch of Neverness to Everness (NTE) on April 29, 2026, introduced a vibrant urban open-world filled with "Espers"—individuals capable of wielding supernatural abilities to combat anomalies. Because team composition is vital for clearing high-level content like the "Anomalous Deep," understanding the various acquisition paths for these characters is the first step for any new player. This guide focuses on maximizing your roster through free progression and the unique [Scarborough Fair](https://ntecodex.com/faq) system.

## Question Restated

![Question Restated](/img/how-to-unlock-characters-neverness-to-everness/inline-1.webp)


When starting your journey in NTE, several questions regarding character acquisition are common:

*   **How do you get new characters (Espers) in Neverness to Everness?** Most characters are obtained through the [Scarborough Fair gacha](https://ntecodex.com/faq), but several are tied to specific milestones in the Hethereau city narrative.
*   **Which characters can be obtained for free through story progression?** The game provides a full functional team of six characters through the main quest, login rewards, and side activities.
*   **How does the Scarborough [Fair gacha](https://ntecodex.com/faq) system work?** Unlike traditional "pull" systems, NTE uses a board-game mechanic involving dice rolls and tile-based rewards.
*   **What is the fastest way to expand your roster at launch?** Utilizing the pre-registration rewards and the one-time S-Rank selector is the most efficient path for new accounts.

## Direct Answer

![Direct Answer](/img/how-to-unlock-characters-neverness-to-everness/inline-2.webp)


Characters in Neverness to Everness (NTE) are unlocked through three primary methods: Main Story progression, limited-time events, and the [Scarborough Fair gacha](https://ntecodex.com/faq) system. Players receive six characters for free: Zero (Starter), Mint (Prologue), Adler (Story), Haniel (Mail/Pre-reg), Aurelia (Login), and Chiz (City Tycoon).

A one-time S-Rank Selector is awarded after completing 50 pulls on the Standard Board, allowing players to choose a high-tier Esper to anchor their team. The gacha system, [Scarborough Fair](https://ntecodex.com/faq), uses a board-game mechanic where players roll Fabricated or Solid Dice to land on character tiles. Notably, NTE features a "No 50/50" system for limited banners, meaning the featured S-Rank character is guaranteed at hard pity, which is set at 90 pulls.

### Summary of Free Characters and Unlock Conditions

| Character | Rank | Element | Unlock Method |
| :--- | :--- | :--- | :--- |
| **Zero** | B | Lakshana | Default Starter Character |
| **Mint** | A | Cosmos | Complete Prologue: "The Midnight Heist" |
| **Adler** | A | Anima | Complete Chapter 1: "Neon Shadows" |
| **Haniel** | A | Incantation | Claim via In-game Mail (Pre-registration Reward) |
| **Aurelia** | A | Psyche | Day 7 Reward of the "Hethereau Welcome" Login Event |
| **Chiz** | S | Chaos | Reach Level 10 in the City Tycoon Management System |

## Detailed Explanation

![Detailed Explanation](/img/how-to-unlock-characters-neverness-to-everness/inline-3.webp)


The process of building a roster in Neverness to Everness is more structured than many other urban RPGs. While the gacha is the primary source for S-Rank powerhouses, the "Free-to-Play" (F2P) path is robust, ensuring every player has a character of each primary element by the time they reach the mid-game.

### Free Characters via Story and Progression

The narrative of NTE acts as a long-form tutorial that rewards you with essential Espers. 

1.  **Zero:** You begin the game as Zero. While Zero starts at B-Rank, they are unique because their rank and element can be shifted as you progress through the main "Anomaly Search" quests.
2.  **Mint:** Mint is your first recruited ally. She is a Cosmos-element DPS who joins your party automatically after you resolve the first major anomaly in the subway system during the prologue.
3.  **Adler:** Adler is an Anima-element support character. To unlock him, you must complete the Chapter 1 finale mission, "Neon Shadows." Adler is essential for early-game survivability due to his shielding capabilities.

### Event and Login Rewards

Beyond the story, the launch celebrations provide two high-value A-Rank characters. **Haniel** is delivered directly to the mailbox for all players who started within the first month of release (or pre-registered). She is an Incantation specialist, making her vital for breaking the shields of "Void-type" enemies.

**Aurelia** is tied to the "Hethereau Welcome" event. This is a standard 7-day login calendar. You do not need to log in on consecutive days, but you must claim seven daily rewards in total to unlock her. Aurelia serves as a Psyche-element healer, a role that is otherwise difficult to fill without spending currency in the gacha.

### The City Tycoon System: Unlocking Chiz

One of the most distinct features of NTE is the **City Tycoon** management simulator. In this mode, you manage real estate and businesses across Hethereau. **Chiz**, an S-Rank Chaos-element character, is the ultimate reward for this system. 

To [unlock Chiz](https://ntecodex.com/faq), you must reach **Tycoon Level 10**. This requires:
*   Completing "Business Anomalies" (management missions).
*   Upgrading the "Antique Shop" to Level 5.
*   Generating a total of [information unavailable] in passive revenue.

Chiz is a powerful sub-DPS who excels in off-field damage, making her a "must-have" for players who do not plan on spending money on the gacha.

### Scarborough Fair: The Board Gacha Explained

The gacha in NTE is stylized as **[Scarborough Fair](https://ntecodex.com/faq)**, a board game where your "pulls" are represented by dice rolls. 

*   **Fabricated Dice:** Used for the Standard Board ("The Eternal City").
*   **Solid Dice:** Used for Limited-Time Banners (e.g., [Nanally](/characters/nanally-guide-nte)’s banner, "The Ichi-daime").
*   **Annulith:** The premium currency used to purchase both types of dice.

When you roll, your piece (a small Chuppa mascot) moves across tiles. 
*   **Journey Together Tiles:** Landing here grants you a random character from the pool.
*   **Hero Chests:** These contain character shards (fragments) or upgrade materials.
*   **Fortune Tiles:** Trigger random events that can boost your next roll or grant bonus currency.

The "No 50/50" system is the most player-friendly aspect of Scarborough Fair. On limited banners, if you reach 90 pulls (the hard pity), you are 100% guaranteed to receive the featured S-Rank character. There is no risk of "losing" to a standard pool character.

### Scarborough Fair Gacha Rates and Pity Thresholds

| Feature | Rate / Value |
| :--- | :--- |
| **S-Rank Character Rate** | 0.6% (Base) |
| **A-Rank Character Rate** | 5.1% |
| **Hard Pity (S-Rank)** | 90 Rolls |
| **Soft Pity Start** | 75 Rolls |
| **50/50 Mechanic** | None (Limited characters are 100% guaranteed at pity) |

### The Standard S-Rank Selector Strategy

After you perform 50 rolls on the Standard Board using Fabricated Dice, the game grants you a **Standard S-Rank Selector**. This is a one-time item that lets you choose one of the six core S-Rank Espers.

#### Standard S-Rank Selector Pool and Roles

| Character | Element | Role | Best For |
| :--- | :--- | :--- | :--- |
| **[Sakiri](/characters/sakiri-nte-free-5-star-guide)** | Lakshana | Main DPS | High-speed fire damage and AOE. |
| **Baicang** | Cosmos | Tank/CC | Freezing enemies and absorbing damage. |
| **Fadia** | Anima | Support | Boosting team energy regeneration. |
| **Hathor** | Incantation | Burst DPS | Massive single-target physical damage. |
| **Jiuyuan** | Chaos | Debuffer | Reducing enemy defense and resistance. |
| **Daffodil** | Psyche | Healer | The strongest pure healer in the standard pool. |

**Strategy Tip:** Do not use your selector immediately. Perform your first 50 rolls and see which A-Rank characters you obtain. If you fail to get a healer, **Daffodil** is the safest pick. If you need a primary damage dealer to carry you through the "Anomalous Deep," **Sakiri** or **Hathor** are the top choices.

### Rerolling for a "God Account"

For competitive players, "rerolling" is a common tactic to start with multiple S-Rank characters. In NTE, this is done via the **"Salting" method**.

1.  Create an account using an email alias (e.g., yourname+1@gmail.com).
2.  Play through the prologue until you unlock the Scarborough Fair menu (roughly 20 minutes).
3.  Collect all pre-registration and mail rewards, which should total approximately 40+ pulls at launch.
4.  Spend all dice on the Limited Banner.
5.  If you do not get the featured S-Rank (like Nanally), log out and repeat with a new alias (yourname+2@gmail.com).

This method allows you to save your S-Rank Selector for a character that complements the limited S-Rank you pulled during the reroll process.

### Checklist: Daily and Weekly Tasks for F2P Pull Currency

To keep pulling for characters, you need a steady stream of Annulith. Follow this checklist:

*   [ ] **Daily Commissions:** Complete 4 tasks for 60 Annulith.
*   [ ] **Weekly Anomaly Cleanup:** Clear 3 stages for 150 Annulith.
*   [ ] **City Tycoon Revenue:** Collect daily profits (small chance for Fabricated Dice).
*   [ ] **Battle Pass (Free Tier):** Reach Level 50 for 5 Fabricated Dice and 2 Solid Dice.
*   [ ] **Hethereau Exploration:** Open "Anomalous Crates" found in the open world (5-10 Annulith each).

## Related

![Related](/img/how-to-unlock-characters-neverness-to-everness/inline-4.webp)


Unlocking characters is only the beginning. To make your Espers effective, you must understand how they interact with the game's combat systems and gear.

### Esper Elements and Team Synergy

Every Esper belongs to one of six elements. Combat in NTE revolves around "Resonance Cycles." When you hit an enemy with a specific element, you build up a meter. Filling this meter triggers a **Resonance Reaction**.

#### Esper Element Reactions and Combat Cycles

| Element 1 | Element 2 | Reaction Name | Effect |
| :--- | :--- | :--- | :--- |
| **Lakshana** | **Cosmos** | Thermal Shock | Deals massive AOE damage and slows enemies. |
| **Anima** | **Incantation** | Overload | Shreds enemy physical shields instantly. |
| **Chaos** | **Psyche** | Mental Collapse | Stuns the target for 5 seconds. |
| **Any** | **Same Element** | Resonance Sync | Increases the active character's damage by 15%. |

Building a team that can trigger at least two different reactions is the key to clearing late-game content.

### Unlocking Arcs (Weapons)

Characters do not use traditional swords or guns. Instead, they equip **Arcs**—manifestations of human memories. Arcs are unlocked through the **Arc Research Program**, which is a separate gacha tab. 

*   **S-Rank Arcs:** Have a 0.7% drop rate.
*   **Signature Arcs:** Every limited S-Rank character has a "Signature Arc" designed for their kit. For example, Nanally's best Arc is undisclosed.
*   **F2P Arcs:** You can craft 4-star Arcs at the "Hethereau Workshop" using materials found in the open world.

### Awakenings and Duplicates

When you pull a duplicate of a character you already own, you receive an **Esper Resonance Crystal**. These are used in the **Awakening** system. 
*   There are 6 levels of Awakening.
*   **Awakening 1 & 2:** Usually provide flat stat boosts (HP/ATK).
*   **Awakening 3 & 6:** Often fundamentally change how a character's skill works (e.g., adding an extra charge to a dash or increasing a buff duration).

For F2P players, focus on Awakening your A-Rank characters, as you will naturally pull more duplicates of them over time.

### Future Character Banners

Following the launch window, Hotta Studio has confirmed several upcoming characters. **Lacrimosa** (a Chaos-element support) is expected to arrive in the second half of the 1.0 update. There are also rumors of a new element, undisclosed, being introduced in the 1.2 expansion.

To stay ahead, always save at least 90 Solid Dice to ensure you can guarantee the next "must-have" character for your specific elemental team.

## Sources

- Neverness to Everness Official Site — https://nte.perfectworld.com/
- Game8 NTE Wiki — https://game8.co/games/Neverness-to-Everness
- IGN: Neverness to Everness Launch Guide
- Prydwen Institute: NTE Character Database
- Reddit: r/NTE_Official — https://www.reddit.com/r/NTE_Official/
- GameSpot: How the Scarborough [Fair Gacha](https://ntecodex.com/faq) Works — https://www.gamespot.com/articles/neverness-to-everness-gacha-guide/1100-6523000/