---
title: "NTE Character Growth Materials Guide: How to Level Up & Ascend"
slug: nte-character-growth-materials-guide
article_type: faq
qa_score: 5.1
word_count: 1633
published_at: "2026-05-23T20:14:10.510164+00:00"
published_url: /faq
sources: []
game: unknown
hero_image: /img/nte-character-growth-materials-guide/hero.webp
inline_images:
  - /img/nte-character-growth-materials-guide/inline-1.webp
  - /img/nte-character-growth-materials-guide/inline-2.webp
  - /img/nte-character-growth-materials-guide/inline-3.webp
  - /img/nte-character-growth-materials-guide/inline-4.webp
inline_image_sections:
  - "Question Restated"
  - "Direct Answer"
  - "Detailed Explanation"
  - "Related"
---

Mastering the urban supernatural landscape of Hethereau requires more than just quick reflexes; it demands a strategic approach to Esper progression. This guide breaks down every essential material and system needed to transform your recruits into high-performance anomaly hunters.

# Neverness to Everness (NTE) Character Growth Materials Guide
> ⚠️ **Notice:** Auto-generated content. May contain approximations or minor inaccuracies in supporting details. Help us improve via the comment section below. _Last reviewed: 2026-05-23._


In the weeks following the April 2026 launch of Neverness to Everness, players have quickly discovered that character power is not a singular metric. Instead, it is a composite of several interconnected systems, each requiring specific resources found throughout the city. To optimize your team, you must balance the acquisition of experience items, breakthrough materials, and high-tech console components. This guide serves as a comprehensive manual for navigating these systems and ensuring your Character Pixels (stamina) are spent efficiently.

## What are the character growth materials in Neverness to Everness?

![What are the character growth materials in Neverness to Everness?](/img/nte-character-growth-materials-guide/inline-1.webp)


Players often find themselves overwhelmed by the variety of icons in their inventory. The core questions surrounding character growth usually involve identifying which materials are required for immediate power spikes versus long-term sustainability. Understanding the different types of character growth materials is the first step toward clearing high-level Anomaly Hunts.

The primary concerns for most players include:
*   What are the different types of character growth materials in Neverness to Everness?
*   Where can players farm EXP items, Ascension materials, and Skill upgrade items?
*   How do progression gates like Hunter Rank and Appraisal Level affect character growth?

In Hethereau, growth is categorized into four distinct pillars: Leveling, Ascension, Esper Abilities, and the Console System. Each pillar has its own dedicated farming node, often tied to Houdinii’s various "Stages" or the mysterious "Rabbit Hole" challenges. Furthermore, the game utilizes a dual-gate system—Hunter Rank (account level) and Appraisal Level (world level)—to ensure players do not outpace the content difficulty, making it vital to know when and where to farm.

## Core Growth Materials and Sources

![Core Growth Materials and Sources](/img/nte-character-growth-materials-guide/inline-2.webp)


Character growth in NTE relies on four main pillars: Leveling (Hunter Guides), Ascension (Anomaly Drops), Esper Abilities (Houdinii's Schemes), and the Console System (Modules/Cartridges). Each of these systems requires a specific investment of Character Pixels to yield the necessary resources.

Primary EXP items are Hunter Guides, which come in Rising, Senior, and Elite tiers. These are farmed at Houdinii's Magic Stage. Ascension, the process of raising a character's level cap, requires materials sourced from Anomaly Hunts and world bosses. This process is strictly gated by Hunter Rank milestones, with the first major breakthrough occurring at Rank 10.

Esper Abilities, which function as the character's active and passive skills, are upgraded using materials found in Houdinii's Schemes. Finally, the Console System acts as a customizable gear set, utilizing Cartridges farmed in the Rabbit Hole and Modules obtained through the Rewind gacha system. Throughout all these processes, Beetle Coins serve as the universal currency for all character upgrades.

| Material Category | Primary Items | Primary Source |
| :--- | :--- | :--- |
| **Character EXP** | Hunter Guides (Rising, Senior, Elite) | Houdinii's Magic Stage |
| **Ascension** | Boss Drops (e.g., Beat King's pick) | Anomaly Hunts |
| **Ability Upgrades** | [Information not yet publicly available as of 2026-05-23] | Houdinii's Schemes |
| **Console Gear** | Cartridges & Modules | Rabbit Hole / Rewind Gacha |
| **Currency** | Beetle Coins | All Urban Activities / Commissions |

## Detailed Breakdown of Character Progression

![Detailed Breakdown of Character Progression](/img/nte-character-growth-materials-guide/inline-3.webp)


The progression of an Esper in Neverness to Everness is a multi-stage process that begins with basic leveling and culminates in the complex optimization of the Console system. To reach the current level cap, players must navigate several "Breakthrough" points that require rare materials from the city’s most dangerous anomalies.

### Character Leveling: Hunter Guides and Beetle Coins

Leveling is the most fundamental way to increase an Esper's base stats, including Health, Attack, and Defense. Unlike some RPGs where combat provides the bulk of experience, NTE relies heavily on the consumption of Hunter Guides. These items are categorized by the amount of experience they provide:

*   **Hunter Guide (Rising):** Provides 1,000 EXP. These are common rewards from early-game side quests and basic Magic Stage runs.
*   **Hunter Guide (Senior):** Provides 5,000 EXP. This is the standard farming target for mid-game players (Hunter Rank 20-35).
*   **Hunter Guide (Elite):** Provides 20,000 EXP. These are most efficiently farmed at higher Appraisal Levels where the Character Pixel-to-EXP ratio is maximized.

To use these guides, you must also spend a corresponding amount of Beetle Coins. As characters reach higher levels (60+), the Beetle Coin cost increases exponentially, making currency management a critical aspect of end-game play.

### Ascension and Breakthrough: Anomaly Hunts and Boss Drops

Once an Esper reaches a level cap (initially level 20), they must undergo Ascension to continue growing. This process requires a mix of common mob drops and specialized "Anomaly" materials. Each Esper is aligned with a specific boss type that drops their required breakthrough item.

Common Ascension Materials include:
*   **Black Tome page:** Often required for Espers with high mental or supernatural utility.
*   **Beat King's pick:** Dropped by the Beat King anomaly; essential for several physical-damage dealers.
*   **Nestbound Bird mask fragment:** Sourced from aerial-type anomalies in the outskirts.
*   **Swallowtail ticket stub:** A rare drop from urban-themed anomalies found in the commercial districts.
*   **Sea Prisoner red liquid:** Obtained from aquatic or fluid-based anomalies.

Ascension is not just about level caps; it often unlocks new passive traits or enhances the base multipliers of an Esper's ultimate ability.

### Esper Ability Enhancement: Navigating Houdinii's Schemes

Upgrading an Esper's abilities (Basic Attack, Skill, and Ultimate) is essential for increasing damage output and utility. These upgrades require specialized materials found in Houdinii's Schemes, which are combat trials themed around specific character classes or elements.

The materials found here are tiered. Lower tiers are used for levels 1-4, while higher-tier "schemes" are required for levels 5-10. Because these nodes rotate or have specific daily bonuses, it is recommended to check the schedule within the "Hethereau Guide" menu to see which ability materials are currently boosted.

### The Console System: Modules, Cartridges, and the Rewind Gacha

The Console system is NTE's version of equipment. Each Esper has a Console that can be fitted with Cartridges and Modules to provide massive stat boosts and set bonuses.

1.  **Cartridges:** These provide the base stats (e.g., Crit Rate, Elemental Damage). They are farmed in the **Rabbit Hole**, a multi-floor dungeon that increases in difficulty.
2.  **Modules:** These act as modifiers or "sub-stats" for the Console. To obtain high-quality Modules, players use the **Rewind** system, which is essentially a gacha for gear components.
3.  **Carrota:** This is the specific currency used for the Rewind gacha. It comes in three rarities:
    *   **Bronze Carrota:** Used for basic Rewind pulls (Common/Uncommon Modules).
    *   **Silver Carrota:** Used for Advanced Rewind pulls (Rare/Epic Modules).
    *   **Gold Carrota:** Used for Legendary Rewind pulls, which guarantee high-tier stat rolls.

### Progression Gates: Hunter Rank and Appraisal Level Tests

You cannot simply farm your way to level 100 on day one. NTE employs "Appraisal Levels" to control the world difficulty and character power. Your Appraisal Level is tied to your Hunter Rank, but it requires a specific "Promotion Test" to increase.

| Hunter Rank | Appraisal Level | Max Character Level | Test Required |
| :--- | :--- | :--- | :--- |
| 1-19 | 0 | 20 | None |
| 20 | 1 | 40 | Appraisal Test I |
| 40 | 2 | 50 | Appraisal Test II |
| 50 | 3 | 60 | Appraisal Test III |
| [Pending] | [Pending] | [Pending] | [Information not yet publicly available as of 2026-05-23] |

Failing to complete the Appraisal Test will cap your Hunter Rank experience, so these should be prioritized as soon as they become available.

### Bond and Affection: Using Gifts for Stat Bonuses

Beyond combat and gear, the relationship between the player and their Espers provides incremental stat bonuses. By giving specific gifts—found in convenience stores or through urban exploration—you increase an Esper's Bond level. Higher Bond levels unlock "Affection Bonuses," which can include small percentages of extra Health or specialized "Urban Traits" that make exploration (like driving or parkour) more efficient.

## Related Progression Systems

![Related Progression Systems](/img/nte-character-growth-materials-guide/inline-4.webp)


While character growth is the priority, your Espers are only as strong as the tools they carry and the city resources you manage.

### Arc (Weapon) Upgrade Materials

In Neverness to Everness, weapons are known as Arcs. Like characters, Arcs require their own set of materials to level up and ascend.
*   **Pigments:** These are the primary experience items for Arcs, found in the **Bubble Can Factory**.
*   **Arc Ascension Materials:** These often overlap with character ascension items but also include unique factory-produced components like "Reinforced Gears" or "Optical Lenses."

### Fons and Urban Resource Management

Fons is a specialized resource used for the more "civilian" side of Hethereau. While not directly used to level up an Esper's attack power, Fons is essential for:
*   **Real Estate:** Purchasing apartments provides rest bonuses that can affect stamina recovery.
*   **Vehicles:** Upgrading your car allows you to reach Anomaly Hunts faster and provides mobile storage for materials.
*   **Shop Investments:** Using Fons to support local businesses can unlock discounted rates for Hunter Guides and Beetle Coins.

### Daily Farming Priority Checklist

To maximize your efficiency in Hethereau, follow this daily priority list:
1.  **Anomaly Pilgrimages:** Complete these daily for a steady supply of Beetle Coins and Hunter Rank experience.
2.  **Spend Character Pixels:** Prioritize Houdinii's Magic Stage if you are below the current level cap, or the Rabbit Hole if your Console needs better Cartridges.
3.  **Anomaly Hunts:** Use your weekly boss claim attempts on bosses like the Beat King or Sea Prisoner to stockpile Ascension materials for future characters.
4.  **Check the Rewind:** Use any accumulated Carrota to fish for better Modules to optimize your main DPS character’s Console.

By following this structured approach to material gathering, you can ensure your team remains capable of handling the increasingly dangerous urban anomalies appearing throughout Hethereau.

## Sources

*   Neverness to Everness Official Site — https://nte.perfectworld.com/
*   NTE Wiki & Database — https://game8.co/games/Neverness-to-Everness
*   IGN: Neverness to Everness Progression Guide — https://www.ign.com/wikis/neverness-to-everness/
*   GameRant: How to Level Up Characters in NTE
*   Prydwen Institute: NTE Character Materials