---
title: "How to Farm NTE Disk Materials: Neverness to Everness Guide"
slug: how-to-farm-nte-disk-materials
article_type: faq
qa_score: 5.1
word_count: 1362
published_at: "2026-05-22T21:16:09.578369+00:00"
published_url: /faq
sources: []
game: unknown
hero_image: /img/how-to-farm-nte-disk-materials/hero.webp
inline_images:
  - /img/how-to-farm-nte-disk-materials/inline-1.webp
  - /img/how-to-farm-nte-disk-materials/inline-2.webp
  - /img/how-to-farm-nte-disk-materials/inline-3.webp
  - /img/how-to-farm-nte-disk-materials/inline-4.webp
inline_image_sections:
  - "Question Restated"
  - "Direct Answer"
  - "Detailed Explanation"
  - "Related"
---

Finding the right equipment is the difference between struggling against high-level Anomalies and clearing Hethereau’s toughest challenges with ease.

# How to Farm NTE Disk Materials: The Ultimate Equipment Guide
> ⚠️ **Notice:** Auto-generated content. May contain approximations or minor inaccuracies in supporting details. Help us improve via the comment section below. _Last reviewed: 2026-05-22._


## Question Restated

![Question Restated](/img/how-to-farm-nte-disk-materials/inline-1.webp)


In *Neverness to Everness* (NTE), "disk materials" is a broad term used by the community to describe the various components required to enhance a character's "Console" and combat performance. To optimize your team, you must distinguish between the three primary equipment categories and the specific currencies required to upgrade them.

### What are Disks in Neverness to Everness?
Disks are the core of the NTE gear system. Unlike traditional RPGs with swords and armor, NTE uses an urban-tech aesthetic where equipment is categorized into Arcs, Cartridges, and Modules. These items are slotted into a character's "Console" interface to provide flat stats, percentage-based bonuses, and unique passive effects. Farming these materials efficiently requires a steady supply of Character Pixels, which act as the game's stamina system.

### Types of Materials You Need to Farm
To fully kit out a character like [Nanally](/characters/nanally-guide-nte) or Mint, you will need to target several different resource nodes:
*   **Arcs:** These function as weapons. Farming Arc materials involves gathering Pigments and specific ascension items to increase their level cap.
*   **Cartridges:** These are the primary gear pieces that provide major stat boosts and set bonuses. They are categorized by rarity, with S-Rank (Gold) being the endgame goal.
*   **Modules:** Often referred to as the "Tetris" system, these are smaller stat-boosters that fit into a grid on the character's Console.
*   **Enhancement Currency:** Manhole is the primary XP material for gear, while Beetle Coins serve as the standard gold-equivalent currency for all transactions.

## Direct Answer

![Direct Answer](/img/how-to-farm-nte-disk-materials/inline-2.webp)


The primary method for farming disk materials in *Neverness to Everness* is through the **Rabbit Hole** domain system. These specialized instances are located throughout the city of Hethereau, specifically in districts like Cape Square and New Herland. Entering a Rabbit Hole stage costs 40 Character Pixels (stamina), and the rewards scale based on your current Hunter Level.

While Cartridges drop directly from these combat challenges, Modules are primarily obtained via the **Rewind** system located within the character Console menu. This system functions as a focused gear-pull mechanic where you spend Carrota Coins (Bronze, Silver, or Gold) to generate new Modules. For general upgrades, Manhole and Beetle Coins are earned through both Rabbit Hole clears and world-based Anomaly Zones.

For maximum efficiency, players should avoid heavy farming of Cartridges until reaching **Hunter Level 30**. At this threshold, S-Rank (Gold) materials begin to drop more frequently, ensuring your Character Pixels are spent on gear that will remain relevant in the endgame.

### Quick Reference for NTE Disk Materials

| Material Name | Primary Source | Use Case |
| :--- | :--- | :--- |
| **Cartridges** | Rabbit Hole Domains | Main equipment pieces; provides set bonuses. |
| **Modules** | Rewind System | Grid-based stat boosters (the "Tetris" system). |
| **Carrota Coins** | Anomaly Zones / Daily Tasks | Currency used to "pull" for new Modules. |
| **Manhole** | Rabbit Hole / Dismantling | The primary XP source for leveling Cartridges. |
| **Beetle Coins** | World Activities / Missions | The standard currency for all upgrade costs. |
| **Pigments** | Boss Challenges / Shops | Used to ascend Arcs (weapons) to higher levels. |
| **Character Pixels** | Natural Regeneration | The stamina required to claim Rabbit Hole rewards. |

## Detailed Explanation

![Detailed Explanation](/img/how-to-farm-nte-disk-materials/inline-3.webp)


Farming in NTE is a structured process that moves from open-world exploration to high-intensity domain clears. Understanding the loop between gathering stamina and spending it in the right districts is essential for progression.

### Farming Cartridges in the Rabbit Hole
The Rabbit Hole is the most important destination for any player looking to improve their combat power. These domains are physical locations in the world that you can fast-travel to once unlocked. 

1.  **Unlocking Stages:** New Rabbit Hole stages unlock as you increase your Hunter Level. Each stage features a specific set of Cartridges.
2.  **Targeted Farming:** Before spending your Character Pixels, check the reward preview. Different Rabbit Holes in Cape Square and New Herland drop different set bonuses (e.g., sets focused on Crit Rate vs. sets focused on Anomaly Damage).
3.  **Difficulty Scaling:** Higher difficulty tiers do not just increase enemy health; they introduce more complex Anomaly modifiers that require specific elemental counters.

### The Rewind System: Farming Modules with Carrota Coins
Modules are unique because they are not dropped directly by enemies in the same way Cartridges are. Instead, you must use the Rewind system. 

The Rewind system allows you to spend Carrota Coins to "generate" Modules of varying shapes and rarities.
*   **Bronze Carrota Coins:** [Information not yet publicly available as of 2026-05-22].
*   **Silver Carrota Coins:** Used for a higher chance at A-Rank (Purple) Modules.
*   **Gold Carrota Coins:** The rarest currency, guaranteed to provide high-tier Modules with better sub-stats.

Because Modules must fit into a limited grid on your character's Console, farming them is a matter of finding the right "shape" to complete your build's Tetris-like layout.

### Where to Get Manhole and Beetle Coins for Upgrades
Even the best S-Rank Cartridge is useless if you cannot level it up. 
*   **Manhole Farming:** While you get Manhole from every Rabbit Hole run, the most efficient way to get large quantities is by dismantling unwanted Cartridges. NTE features a generous dismantling strategy: you receive a significant portion of the XP invested back when recycling gear, allowing you to transition from early-game B-Rank gear to late-game S-Rank gear without losing all your progress.
*   **Beetle Coins:** These are earned from almost every activity in Hethereau. However, if you run low, the "Anomaly Sweep" missions in the open world provide the highest Beetle Coin-to-time ratio.

### Arc Disk Materials: Farming Pigments and Ascension Items
Arcs (weapons) require a different set of materials for ascension. To push an Arc past level 20, 40, or 60, you will need Pigments. These are typically found in Boss Challenges located in the more dangerous outskirts of the city. Each Arc type (e.g., those used by Nanami or Nadal) requires a specific color or type of Pigment, so players should check the "Ascension" tab in the Arc menu to see which boss currently holds the necessary drops.

### Efficiency Tips: When to Start the Gold Gear Grind
Efficiency is key in NTE because Character Pixels are a limited resource.
*   **Hunter Level 1-20:** Focus on story missions and world exploration. Use whatever B-Rank (Blue) or A-Rank (Purple) gear you find naturally.
*   **Hunter Level 20-30:** Start doing daily Rabbit Hole runs to keep your main team geared, but do not use "Pixel Refills" (stamina potions) yet.
*   **Hunter Level 30+:** This is when S-Rank (Gold) gear becomes a common drop. This is the optimal time to spend your saved Pixel Refills to farm for specific Cartridge sets with ideal sub-stats.

## Related

![Related](/img/how-to-farm-nte-disk-materials/inline-4.webp)


As you perfect your gear farming routine, you may want to look into how specific characters utilize these materials. Different characters benefit from vastly different Cartridge configurations.

### Best Cartridge Sets for Every Role
Characters in NTE generally fall into roles like Main DPS, Support, or Breaker. 
*   **Main DPS (e.g., Nanally):** Usually requires the [Information not yet publicly available as of 2026-05-22] set which boosts Crit Damage.
*   **Breakers:** Focus on sets that increase "Anomaly Pressure" to shield-break enemies faster.
*   **Supports:** Look for sets that provide team-wide buffs or energy regeneration when the character is off-field.

### How to Increase Your Hunter Level Fast
Since the best farming nodes are locked behind Hunter Levels, reaching Level 30 should be your priority. The fastest way to gain Hunter XP is by completing "City Errands" and clearing the "Anomaly Handbook" tasks. These provide massive chunks of XP without requiring Character Pixels.

### NTE Gacha Guide: Dice and Board Mechanics
While you farm for gear, you will also be interacting with the "Scarborough Fair" gacha system to pull for new Arcs and characters. Unlike traditional gacha, NTE uses a unique board-game-inspired mechanic where players roll dice to progress toward a guaranteed "Resonance" (pity). Understanding how to balance your spend between the character banners (like Nanally's "The Ichi-daime") and the weapon banners is crucial for long-term account health.

## Sources
*   Neverness to Everness Official Site — https://nte.hotta.studio/
*   Game8 Neverness to Everness Guide — https://game8.co/games/Neverness-to-Everness
*   IGN: Neverness to Everness Everything We Know — https://www.ign.com/articles/neverness-to-everness-everything-we-know
*   Reddit: r/NTE_Official — https://www.reddit.com/r/NTE_Official/
*   Prydwen Institute: NTE Character and Gear Database — https://www.prydwen.gg/nte/