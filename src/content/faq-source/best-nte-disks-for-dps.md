--
title: "Best NTE Disks for DPS: Top Arcs and Cartridges Guide"
slug: best-nte-disks-for-dps
article_type: faq
qa_score: 5.5
word_count: 1721
published_at: "2026-05-23T09:51:47.041585+00:00"
published_url: /faq
sources: []
game: unknown
hero_image: /img/best-nte-disks-for-dps/hero.webp
inline_images
  - /img/best-nte-disks-for-dps/inline-1.webp
  - /img/best-nte-disks-for-dps/inline-2.webp
  - /img/best-nte-disks-for-dps/inline-3.webp
  - /img/best-nte-disks-for-dps/inline-4.webp
inline_image_sections
  - "Question Restated"
  - "Direct Answer"
  - "Detailed Explanation"
  - "Related"
--

Selecting the right combination of Arcs and Cartridges is the defining factor between clearing Hethereau’s toughest anomalies and hitting a progression wall. With the global launch of Neverness to Everness (NTE) on April 29, 2026, players have quickly discovered that "Disks"—the community term for the dual-gear system of weapons and equipment—require precise synergy to unlock maximum DPS potential.

# Best NTE Disks for DPS: Top Arcs and Cartridges Guide
> ⚠️ **Notice:** Auto-generated content. May contain approximations or minor inaccuracies in supporting details. Help us improve via the comment section below. _Last reviewed: 2026-05-23._


## Question Restated

![Question Restated](/img/best-nte-disks-for-dps/inline-1.webp)


In Neverness to Everness, the term "Disks" is frequently used by the player base to describe the two primary pillars of character builds: Arcs (the weapon system) and Cartridges (the gear or relic system). While the game officially categorizes these separately, their integration within the character's "Console" interface means that optimizing one requires a deep understanding of the other. For any Main DPS or Sub-DPS character, choosing the correct Disk configuration is the most significant way to scale damage output in the Version 1.0 meta.

### What are Disks in Neverness to Everness?

Arcs are the primary "Weapon Disks" that dictate a character's base Attack (ATK) and provide unique passive abilities. These are categorized into three physical states: Plasma, Solid, and Gas. Each state interacts differently with character kits, often favoring specific combat styles such as rapid-fire projectiles or heavy melee strikes.

Cartridges, on the other hand, function as the "Gear Disks." These are equipped in sets to provide stat boosts and powerful set bonuses. Unlike traditional gear systems, Cartridges in NTE are slotted into the Console, where their effectiveness is further amplified by the Module system. To maximize a DPS character, you must align the Arc’s passive triggers with the Cartridge’s set requirements.

### Why DPS Optimization Matters

The combat in NTE revolves around the "Esper Cycle," where triggering elemental reactions like Blossom, Scorch, and Hex is essential for shredding enemy resistances. If your DPS Disks are not optimized, you will find it increasingly difficult to trigger these reactions before the enemy’s "Stability Bar" recovers. In the current endgame meta, specifically within the high-level Urban Anomalies, raw ATK is no longer sufficient. High-tier builds now focus on DEF-ignore mechanics, Crit ratios, and Elemental DMG bonuses to bypass the scaling armor of boss-level entities.

## Direct Answer

![Direct Answer](/img/best-nte-disks-for-dps/inline-2.webp)


The current Version 1.0 meta for Neverness to Everness prioritizes Arcs and Cartridges that offer multiplicative damage scaling rather than flat stat increases. While several options exist, the following Disks have emerged as the gold standard for damage dealers as of May 23, 2026.

**Top-Tier DPS Arcs (Weapon Disks):**
*   **S-Tier:** [Information not yet publicly available as of 2026-05-23] — Currently regarded as the Best-in-Slot (BiS) for Anima-based attackers like [Nanally](/characters/nanally-guide-nte) due to its high Crit Rate and follow-up damage amplification.
*   **S-Tier:** [Information not yet publicly available as of 2026-05-23] — The premier choice for Cosmos-element DPS, offering significant DMG% buffs that scale with the character's movement speed.
*   **A-Tier:** [Information not yet publicly available as of 2026-05-23] — A highly versatile Arc that rewards consistent basic attacks with stacking ATK and Crit DMG bonuses.

**Top-Tier DPS Cartridge Sets:**
*   **Lost Radiance:** The most universal DPS set, providing a massive 25% DEF-ignore bonus when a 4-piece set is equipped.
*   **Fireflies and the Forest:** The definitive set for Anima DPS characters, focusing on stacking Crit DMG through elemental triggers.
*   **Devil's Blood: Curse:** A specialized set designed for Damage-over-Time (DoT) and condition-based attackers like Jiuyuan.

### S-Tier DPS Arcs at a Glance

| Arc Name | Primary Stat | Key Passive Effect |
| :--- | :--- | :--- |
| [Name pending] | Crit Rate | Increases follow-up damage by [exact value pending] after using an Esper Skill. |
| [Name pending] | Crit DMG | Grants a DMG% bonus based on the distance traveled during combat. |
| [Name pending] | Elemental DMG | Shreds enemy Resistance by a fixed percentage upon triggering an Esper Reaction. |
| [Name pending] | ATK% | Increases ATK speed and reduces Esper Skill cooldowns on hit. |
| [Name pending] | Energy Regen | Converts excess Energy into a flat DMG bonus for the Ultimate ability. |

### Must-Have Cartridge Sets for Damage

| Cartridge Set | 2-Piece Bonus | 4-Piece Bonus |
| :--- | :--- | :--- |
| **Lost Radiance** | +15% ATK | Ignores 25% of the target's DEF for 10s after an Ultimate. |
| **Fireflies and the Forest** | +10% Anima DMG | Increases Crit DMG by 8% per stack (up to 7 stacks) on Anima trigger. |
| **Devil's Blood: Curse** | +20% Status Accuracy | Increases DoT damage by 40% against enemies with Hex or Scorch. |

## Detailed Explanation

![Detailed Explanation](/img/best-nte-disks-for-dps/inline-3.webp)


The complexity of NTE’s gear system lies in the interaction between the physical state of the Arc and the "Tetris" layout of the Console Modules. Understanding these layers is vital for moving beyond basic builds into endgame-ready optimization.

### Arc Passives and Refinement Priorities

Arcs in Neverness to Everness are more than just stat sticks; they define the character's combat rhythm. For example, characters like Nanally rely on Arcs that enhance her "Anima" state. When selecting an Arc, you must match the physical type—Plasma, Solid, or Gas—to the character’s innate scaling. 

Refining Arcs (using duplicate copies) is particularly impactful in NTE. Unlike other games where refinement only offers marginal gains, NTE Arcs often unlock secondary passive tiers at Refinement Level 3 (R3). For DPS Arcs, these tiers usually involve "Execution" mechanics, where damage is increased by [information unavailable] against enemies below 30% HP. If you are a F2P player, focusing on refining A-rank Arcs like [Information not yet publicly available as of 2026-05-23] can often outperform a base R1 S-rank Arc.

### Cartridge Set Mechanics: 2-Piece vs. 4-Piece

The Cartridge meta is currently dominated by the **Lost Radiance** set. The reason for its supremacy in Version 1.0 is the 25% DEF-ignore mechanic. In NTE, defense scaling for enemies in the "Deep Anomaly" floors is exponential. While a flat ATK set like **Urban Vanguard** (often used in the early game) provides a visible boost to your stat sheet, it falls off significantly when facing armored bosses.

For elemental specialists, the **Fireflies and the Forest** set is the high-ceiling alternative. This set requires the player to maintain "stacks" by consistently dealing Anima damage. At maximum stacks, it provides a 56% Crit DMG buff. However, maintaining this requires a team composition that can provide high "Esper Particle" generation to ensure the main DPS never drops their elemental infusion.

### Mastering the Console: Module Types and Shapes

The "Console" is where your Disks are actually powered up. It uses a grid-based system where you must fit "Modules" of various shapes (Type II, III, and IV). 
*   **Type II Modules:** Generally provide offensive stats like Crit Rate and ATK.
*   **Type III Modules:** Focus on utility, such as Cooldown Reduction and Energy Regen.
*   **Type IV Modules:** These are the rarest and provide massive boosts to Elemental DMG and Armor Penetration.

The "Tetris" aspect comes from the fact that certain Cartridges and Arcs grant bonus stats if you fill specific patterns in the Console. For instance, Nanally gains a 6% Crit Rate bonus for every Type II module placed in a vertical column. If your Module layout is messy, you lose out on these "Resonance" bonuses, which can account for up to 20% of your total damage output. Using **Carrota Coins** to "Rewind" or reshuffle Module shapes is a necessary part of the endgame grind to ensure a perfect fit.

### Optimal Main Stats and Substats for DPS

When farming Cartridges using your **Character Pixels** (the game's stamina equivalent), you should look for specific main stat distributions
1.  **Disk 1 & 2:** Fixed flat ATK and HP (Focus on substats here).
2.  **Disk 3:** ATK% or Energy Regen.
3.  **Disk 4:** Elemental DMG Bonus (Anima, Cosmos, etc.).
4.  **Disk 5:** Crit Rate or Crit DMG (Aim for a 1:2 ratio).

In the current 2026 patch, the community has found that stacking pure ATK% leads to heavy diminishing returns once you surpass 2,500 total ATK. At that point, investing in "Damage Multipliers" such as Elemental DMG% and DEF-ignore provides a much higher "effective damage" increase per stat roll.

## Related

![Related](/img/best-nte-disks-for-dps/inline-4.webp)


Building a top-tier DPS requires looking beyond just the main attacker. The synergy between your Main DPS Disks and your support team’s equipment can amplify your damage by over 100%.

### Best Disks for Nanally (Anima Main DPS)

Nanally remains the most popular DPS since the "The Ichi-daime" banner launched. Her best build involves
*   **Arc:** [Information not yet publicly available as of 2026-05-23].
*   **Cartridge:** 4-piece **Fireflies and the Forest**.
*   **Console Priority:** Type II Modules (Square shapes) to trigger her Crit Rate Resonance.

### Best Disks for Chiz and Esper Zero (Cosmos)

Cosmos characters thrive on "Stability Damage," which allows them to break enemy guards faster.
*   **Arc:** [Information not yet publicly available as of 2026-05-23].
*   **Cartridge:** 4-piece **Lost Radiance** or 2-piece **Cosmos Echo** / 2-piece **Lost Radiance**.
*   **Strategy:** Focus on "Break Efficiency" substats to trigger the Blossom reaction more frequently.

### F2P Friendly DPS Disk Options

For players who haven't pulled S-rank Arcs from the gacha, there are several powerful alternatives
*   **OraOra!:** This Arc can be purchased from the Hethereau Reputation Shop. It provides a solid ATK boost and is easy to Max Refine (R5).
*   **Song of the Whale:** A defensive/offensive hybrid Arc that is excellent for newer players struggling with survivability in the open world.
*   **Cartridge Farming:** Stick to the **Standard Resonance** sets available in early-game domains until you can comfortably clear the level 60 Cartridge trials.

### Support Disks that Buff Your DPS

Your supports (like [Sakiri](/characters/sakiri-nte-free-5-star-guide) or Haniel) should run the **Speedy Hedgehog** Cartridge set. The 4-piece bonus of this set grants the entire team a 15% ATK buff and a 10% Movement Speed buff for 12 seconds after the wearer uses an Esper Skill. When timed with your Main DPS’s Ultimate, this creates a "Burst Window" that is essential for clearing timed challenges.

Additionally, triggering the correct **Esper Cycle** is crucial. For example, using a support to apply "Scorch" before your Main DPS applies "Anima" will trigger a "Combustion" reaction, which increases the damage of your next 3 hits by [information unavailable]. Ensuring your Disks support these reaction-heavy playstyles is the hallmark of a pro NTE player.

## Sources

- Neverness to Everness Official Site — https://nte.hotta.com/
- Hotta Studio Official Discord — https://discord.gg/nteofficial
- IGN: Neverness to Everness Combat and Gear Guide
- Game8: NTE Best Weapons and Disks Tier List — https://game8.co/games/NTE/anticles/best-disks-tier-list
- Reddit: r/NTE_Official Theorycrafting Megathread — https://www.reddit.com/r/NTE_Official/comments/version1-0-meta-discussion/