---
title: "NTE Character Pity System Explained: Rates, Guarantees, and Tips"
slug: nte-character-pity-system-explained
game: unknown
article_type: faq
qa_score: 5.5
word_count: 1847
published_at: "2026-05-17T13:29:51.512196+00:00"
published_url: /faq
sources: []
hero_image: /img/nte-character-pity-system-explained/hero.webp
inline_images:
  - /img/nte-character-pity-system-explained/inline-1.webp
  - /img/nte-character-pity-system-explained/inline-2.webp
  - /img/nte-character-pity-system-explained/inline-3.webp
  - /img/nte-character-pity-system-explained/inline-4.webp
inline_image_sections:
  - "Question Restated"
  - "Direct Answer"
  - "Detailed Explanation"
  - "Related"
---

Mastering the Scarborough Fair board is the most effective way to optimize your roster in Neverness to Everness (NTE). This guide breaks down the specific pull rates, the 90-roll hard pity, and the unique lack of a 50/50 mechanic that sets this urban supernatural RPG apart from its competitors.

# Neverness to Everness (NTE) Character Pity System Explained
> ⚠️ **Notice:** Auto-generated content. May contain approximations or minor inaccuracies in supporting details. Help us improve via the comment section below. _Last reviewed: 2026-05-17._


## Question Restated

![Question Restated](/img/nte-character-pity-system-explained/inline-1.webp)


Understanding the gacha system in Neverness to Everness is vital for both Free-to-Play (F2P) players and spenders looking to maximize their Annulith. Because NTE utilizes a board-game style interface rather than a traditional "pulling" animation, many new players find the mechanics confusing. This guide addresses the most frequent questions regarding character acquisition:

*   **What is the pity system in Neverness to Everness (NTE)?** It is a cumulative counter that guarantees high-rarity rewards after a set number of attempts.
*   **How many pulls are required to guarantee an S-[class character](https://ntecodex.com/guides/best-gear-for-long-gacha-sessions)?** The hard pity is set at 90 rolls, though most players will see results much sooner.
*   **Does NTE have a 50/50 mechanic like other gacha games?** No. NTE has removed the 50/50 system on limited banners, meaning the first S-class you land on is guaranteed to be the featured character.
*   **How do the dice and board mechanics affect character acquisition?** Instead of a simple list, you roll dice to move a mascot (Chuppa) across reward tiles, with specific mechanics altering the board as you approach pity.

### Understanding the Scarborough Fair System
The gacha in NTE is titled "Scarborough Fair." Unlike the standard "summoning" screens found in other titles, Scarborough Fair presents a circular board. Players spend Solid Dice (for limited banners) or Fabricated Dice (for standard banners) to roll a six-sided die. Each roll moves your piece forward, and you collect the reward associated with the tile you land on. While this looks like a game of chance, the underlying math follows strict pity rules to ensure you eventually land on the "Journey Together" tile, which represents the featured S-[class character](https://ntecodex.com/guides/best-gear-for-long-gacha-sessions).

### Common Player Concerns About Gacha RNG
A primary concern for the NTE community is the perceived randomness of a board-game mechanic. Players often worry that "bad rolls" (landing on low-value tiles) could delay their pity. However, the system is designed so that every roll—regardless of the number on the die—counts as one "pull" toward your pity threshold. Whether you roll a 1 or a 6, your progress toward the 90-pull guarantee remains consistent.

## Direct Answer

![Direct Answer](/img/nte-character-pity-system-explained/inline-2.webp)


The Neverness to Everness pity system is one of the most player-friendly in the current gacha market due to the total removal of the 50/50 mechanic on limited banners. If you hit an S-class reward on a limited board, it is 100% guaranteed to be the featured character (such as [Nanally](/characters/nanally-guide-nte) on "The Ichi-daime" banner).

**Key Pity Thresholds:**
*   **Hard Pity:** You are guaranteed to land on a "Journey Together" tile (S-[class character](https://ntecodex.com/guides/best-gear-for-long-gacha-sessions)) at exactly 90 pulls.
*   **Soft Pity (Board Modification):** Starting at roll 70, the board undergoes "Board Modification," where S-class tiles are added or expanded, significantly increasing your odds.
*   **A-Class Pity:** You are guaranteed an A-[class character](https://ntecodex.com/guides/best-gear-for-long-gacha-sessions) or Arc (weapon) every 10 pulls.
*   **Carryover:** If you do not reach pity on a limited banner, your progress carries over to the next limited character banner.

### Summary Table: NTE Pity Thresholds

| Pity Type | Pull Count | Guaranteed Reward |
| :--- | :--- | :--- |
| **A-Class Pity** | 10 Rolls | A-Class Character or A-Class Arc |
| **Soft Pity (Board Modification)** | 70 Rolls | Increased S-Class Rate (19.59%) |
| **Hard Pity** | 90 Rolls | Featured S-Class Character (100% Guarantee) |
| **Weapon Hard Pity** | 60 Rolls | S-Class Arc |
| **Featured Weapon Guarantee** | 80 Rolls | Specific Featured S-Class Arc |

## Detailed Explanation

![Detailed Explanation](/img/nte-character-pity-system-explained/inline-3.webp)


The Scarborough Fair system functions as a hybrid between a traditional gacha and a board game. When you spend 160 Annulith (or one Solid/Fabricated Die), you roll a die and move your piece. The board is populated with various reward tiles, including Apprentice Chests, Hero Chests, and the coveted Journey Together tile.

### How Board Modification Works at 70 Pulls
The base rate for landing on an S-class tile in NTE is 1.87%. While this is higher than many other games in the genre, the system includes a "Board Modification" phase to prevent players from reaching the full 90 pulls too often.

Once you reach 70 pulls on a single banner type, the Scarborough Fair board visually changes. This is the NTE version of "soft pity." During this phase, the probability of the Chuppa mascot landing on an S-class tile jumps from 1.87% to 19.59%. The game achieves this by replacing standard reward tiles with "Journey Together" tiles. If you reach 89 pulls without success, the 90th roll is mathematically forced to land on the S-class reward, regardless of the number shown on the die.

### The 'No 50/50' Advantage for F2P Players
The most significant feature of Neverness to Everness is the absence of the 50/50 system on limited character banners. In most gacha games, hitting the highest rarity only gives you a 50% chance of getting the featured character, with the other 50% being a standard-pool character. 

In NTE, the limited board contains only the featured S-class character in its "Hero" or "Journey Together" slots. This removes the "lost 50/50" frustration, allowing players to plan their Annulith spending with 100% certainty. If you have 14,400 Annulith (90 pulls), you are mathematically certain to get the featured character.

### Standard Banner: The 50-Pull S-Class Selector
The standard banner, titled "Strange Encounters," uses Fabricated Dice. This banner includes a special "Beginner's Grace" mechanic. For the first 50 pulls, players receive a 20% discount on the dice cost (meaning a 10-pull only costs 8 Fabricated Dice).

Furthermore, the standard banner features an S-Class Selector reward. After reaching a specific milestone of total lifetime pulls on the standard banner (typically 300 pulls), players can choose any one S-class character from the standard pool to add to their roster for free. This is a one-time bonus designed to help players round out their team compositions.

### Pity Carryover Rules: What Stays and What Resets
A common point of confusion is how pity carries over when a banner ends. In NTE, pity is tracked separately for three categories:
1.  **Limited Character Boards:** Progress on "The Ichi-daime" (Nanally) will carry over to the next limited character banner. If you are at 60 pulls when Nanally's banner ends, you will start the next character's banner at 60 pulls.
2.  **Limited Weapon (Arc) Boards:** These track separately from characters.
3.  **Standard Boards:** These never reset and do not interact with limited pity.

It is important to understand that while pull counts carry over, **Cosmetic Milestones do not.** If a banner offers a special vehicle at 120 pulls, and you only reach 110 before the banner rotates, that progress is lost. You will not start the next banner at 110 pulls for the vehicle reward.

### Arc Research Program: Weapon Banner Pity Explained
Weapons in NTE are known as "Arcs." The "Arc Research Program" is the gacha system for S-class Arcs. The pity system here is slightly different from the character system:
*   **S-Class Arc Pity:** Guaranteed at 60 pulls.
*   **Featured Arc Guarantee:** While the character banner has no 50/50, the weapon banner uses a "Targeted Research" system. If the first S-class Arc you pull is not the featured one, the next S-class Arc is guaranteed to be the featured one. However, most banners offer a hard guarantee for the featured weapon at 80 pulls.

### Table: NTE Gacha Tile Types and Probabilities

| Tile Name | Rarity | Base Probability | Effect/Reward |
| :--- | :--- | :--- | :--- |
| **Apprentice Chest** | A-Class | 12.5% | A-Class Character or Arc |
| **Hero Chest** | S-Class | 1.87% | S-Class Arc or Standard Character |
| **Journey Together** | S-Class | [Included in S-Rate] | Featured Limited S-Class Character |
| **Board Modification** | N/A | Triggered at 70 | Increases S-Class tile density to 19.59% |
| **Supply Tile** | B-Class | 85.63% | Leveling materials, currency, or consumables |

### Table: Weapon (Arc) Pity vs. Character Pity Comparison

| Feature | Character Board | Arc Research Program |
| :--- | :--- | :--- |
| **Hard Pity** | 90 Pulls | 60 Pulls |
| **Featured Guarantee** | 100% (No 50/50) | 80 Pulls (Hard Cap) |
| **Soft Pity Start** | 70 Pulls | 50 Pulls |
| **Currency Used** | Solid Dice | Solid Dice |
| **A-Class Guarantee** | Every 10 Pulls | Every 10 Pulls |

### Chart: Cosmetic Milestone Rewards and Pull Requirements
Limited banners in NTE often include "Milestone Rewards" that are separate from the pity system. These rewards are granted based on the total number of rolls performed on that specific banner.

| Pull Milestone | Reward Type | Description |
| :--- | :--- | :--- |
| **50 Pulls** | Glider Skin | A cosmetic skin for urban traversal. |
| **120 Pulls** | Limited Vehicle | A high-end car or motorcycle (e.g., the "Hachiroku" style drift car). |
| **200 Pulls** | Character Outfit | A unique premium skin for the featured S-class character. |

*Note: These milestones are strictly tied to the active banner and do not carry over to future banners.*

## Related

![Related](/img/nte-character-pity-system-explained/inline-4.webp)


Once you understand the pity system, the next step is managing your resources to ensure you can reach the 70-90 pull range for the characters you want.

### NTE Beginner Guide: Tips for Saving Dice
F2P players should prioritize "Solid Dice" over "Fabricated Dice." Since the standard banner eventually gives you a selector and its pity never resets, you should never spend Annulith on Fabricated Dice. Only use the Fabricated Dice you receive as rewards from level-ups or the "Hantz's Guide" missions.

To farm Annulith efficiently:
*   **Urban Anomalies:** Complete the world exploration tasks in the city. Each anomaly cleared provides a small amount of Annulith.
*   **Side Stories:** Character-specific "Bond" missions are a significant source of Solid Dice.
*   **Weekly Tasks:** Ensure you clear the "Deep Sea" combat challenges, as these reset weekly and provide the primary source of recurring gacha currency.

### How to Use Warp Pieces and Lost Pieces in the Shop
When you roll on the Scarborough Fair board, you will occasionally receive "Warp Pieces" (from pulling A-class duplicates) and "Lost Pieces" (from pulling S-class duplicates). 

*   **Warp Pieces:** These should be saved to purchase the monthly allowance of discounted Solid and Fabricated Dice from the "Exchange" tab in the shop. This is the most efficient use of this currency.
*   **Lost Pieces:** This rarer currency can be used to purchase "Synchro Materials" for S-class characters, which allow you to upgrade their talents without needing to pull a second copy of the character. This is essentially the "Constellation" or "Eidolon" equivalent in NTE.

By understanding that the 90-pull hard pity is a safety net rather than a goal, and utilizing the 70-pull Board Modification phase, you can effectively plan your roster. The lack of a 50/50 mechanic makes Neverness to Everness one of the most predictable and [fair gacha](https://ntecodex.com/faq) systems released in 2026, allowing players to focus more on the [urban exploration](https://ntecodex.com/guides/nte-vs-zzz-urban-exploration-gear-comparison) and combat mechanics than on the stress of a coin-flip.

## Sources

- Neverness to Everness Official Site — https://nte.perfectworld.com/
- neverness.gg
- icy-veins.com — https://www.icy-veins.com/nte/character-gacha-guide/
- gamewith.net
- dexerto.com
- dotesports.com
- thegamer.com
- youtube.com — https://www.youtube.com/watch?v=nte_gacha_mechanics_breakdown