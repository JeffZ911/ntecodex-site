---
title: How to Increase Resonance Level in Neverness to Everness (NTE)
slug: nte-how-to-increase-resonance-level
game: unknown
article_type: faq
qa_score: 4.7
word_count: 2023
published_at: "2026-05-15T07:59:28.985433+00:00"
published_url: /faq
sources: []
hero_image: /img/nte-how-to-increase-resonance-level/hero.webp
inline_images:
  - /img/nte-how-to-increase-resonance-level/inline-1.webp
  - /img/nte-how-to-increase-resonance-level/inline-2.webp
  - /img/nte-how-to-increase-resonance-level/inline-3.webp
  - /img/nte-how-to-increase-resonance-level/inline-4.webp
inline_image_sections:
  - "Question Restated"
  - "Direct Answer"
  - "Detailed Explanation"
  - "Related"
---

Maximizing a character's potential in the urban supernatural landscape of Hethereau requires more than just standard leveling; it requires a deep understanding of the Resonance system. To push your team toward end-game content, you must learn how to increase Resonance Level to unlock the powerful R1 and R2 passives that define the meta.

# How to Increase Resonance Level in Neverness to Everness (NTE)
> ⚠️ **Notice:** Auto-generated content. May contain approximations or minor inaccuracies in supporting details. Help us improve via the comment section below. _Last reviewed: 2026-05-15._


## Question Restated

![Question Restated](/img/nte-how-to-increase-resonance-level/inline-1.webp)


In Neverness to Everness (NTE), Resonance Level represents the vertical progression of a specific character’s power through the activation of Awakening nodes. While most players focus on increasing a character's base level to improve raw stats like HP and ATK, Resonance Level focuses on enhancing the character's kit by adding new mechanics, reducing cooldowns, or increasing skill multipliers. 

Increasing Resonance Level is a long-term goal that transforms a character from a basic unit into a specialized powerhouse. For example, a character at Resonance Level 0 (R0) might have a standard skill rotation, whereas that same character at Resonance Level 1 (R1) could gain the ability to reset their primary skill upon performing a successful dodge or elemental reaction.

### What is Resonance Level in NTE?

Resonance Level is the tier system associated with the Awakening menu. Every character has six Awakening nodes, labeled A1 through A6. As you activate these nodes using specific materials, your Resonance Level increases at specific milestones. 

Activating these nodes does more than just provide a flat stat boost. Each node typically targets a specific part of the character's "Esper" abilities. Some nodes might increase the duration of an elemental infusion, while others might provide energy regeneration for the entire team when the character is off-field.

### Resonance vs. Hunter Level: Key Differences

It is easy for new players to confuse Resonance Level with other progression systems in NTE. However, they serve entirely different purposes and are increased through different gameplay loops.

| Feature | Resonance Level | Hunter Level | Appraisal Level |
| :--- | :--- | :--- | :--- |
| **Primary Purpose** | Character-specific power and passive unlocks. | Overall account progression and level gating. | World difficulty and loot quality scaling. |
| **How to Increase** | Using Mind Shards and Awakening nodes (A1-A6). | Completing missions, Daily Commissions, and spending Stamina. | Completing "Appraisal Rank" trials at specific Hunter Levels. |
| **Impact** | Skill enhancements, cooldown resets, and R1/R2 buffs. | Unlocks new story chapters and increases character level caps. | Higher drop rates for gold-tier Cassettes and Arcs. |
| **Reset/Cap** | Permanent per character; capped at R2 (A6). | Permanent per account; currently capped at —. | Increases world difficulty; can be manually lowered in settings. |

## Direct Answer

![Direct Answer](/img/nte-how-to-increase-resonance-level/inline-2.webp)


The primary way to increase Resonance Level in Neverness to Everness is by activating Awakening nodes (A1 through A6) within the character's upgrade menu. This process requires character-specific items known as Mind Shards, which are consumed to "awaken" the character's latent potential.

### The Short Answer: Mind Shards and Awakening

To increase your Resonance Level, you must acquire Mind Shards for the specific character you wish to upgrade. Once you have the required number of shards, navigate to the character screen, select "Awakening," and activate the next available node. You cannot skip nodes; they must be activated in a specific sequence, although some characters offer branching choices within the A1-A3 and A4-A6 brackets.

Mind Shards are primarily sourced from:
*   **The Scarborough Fair:** The game's gacha system where pulling duplicate characters grants shards.
*   **The Warp Exchange:** A shop where "Warp Pieces" (earned from gacha pulls) can be traded for specific character shards.
*   **City Tycoon:** The economic simulation mode where reaching specific business milestones provides free shards for certain characters.

### Resonance Level Milestones

While there are six total Awakening nodes, the community and the game UI categorize character power into three main Resonance states:

*   **Resonance Level 0 (R0):** The base state of a character when first obtained. No Awakening nodes are active.
*   **Resonance Level 1 (R1):** Achieved immediately upon activating the **third Awakening node (A3)**. This usually unlocks the first major gameplay-altering passive.
*   **Resonance Level 2 (R2):** Achieved upon activating **all six Awakening nodes (A6)**. This represents the character's maximum potential, often providing massive team-wide buffs or devastating ultimate ability enhancements.

| Milestone | Requirement | Benefit Type |
| :--- | :--- | :--- |
| **A1 - A2** | 1 Mind Shard each | Minor stat boosts and skill level increases. |
| **R1 (A3)** | 1 Mind Shard | **Major Passive Unlock:** Often changes the character's core mechanic. |
| **A4 - A5** | 1 Mind Shard each | Significant stat scaling and utility improvements. |
| **R2 (A6)** | 1 Mind Shard | **Ultimate Enhancement:** Maximum power ceiling, often adding team-wide utility. |

## Detailed Explanation

![Detailed Explanation](/img/nte-how-to-increase-resonance-level/inline-3.webp)


The Resonance system in Neverness to Everness is designed to reward players for investing in their favorite characters over time. Unlike some systems that only provide linear stat growth, NTE’s Awakening nodes often introduce "mechanical shifts" that change how a character functions within the Esper Cycle.

### The Awakening Node System Explained

When you enter the Awakening menu, you will see a hexagonal grid or a linear path (depending on the character's rarity) containing six nodes. 

To activate a node, you need:
1.  **Mind Shards:** 1 per node for most characters.
2.  **Analith:** A common upgrade essence found in [urban exploration](https://ntecodex.com/guides/nte-vs-zzz-urban-exploration-gear-comparison) and combat challenges.
3.  **Fons:** A specialized currency used for high-level character modifications.

While you must eventually activate all six nodes to reach R2, the path between A1 and A3 sometimes allows for slight prioritization of stats (e.g., choosing between an ATK-focused node or a Crit-focused node first). However, the R1 milestone remains locked until the third node is fully energized.

### Farming Mind Shards: Gacha, Shops, and Events

The most common way to increase Resonance Level is through the [Scarborough Fair gacha](https://ntecodex.com/faq) system. When you pull on a banner—such as the limited-time "The Ichi-daime" banner for [Nanally](/characters/nanally-guide-nte)—any duplicate of a character you already own is automatically converted into Mind Shards.

*   **Fabricated Dice:** Used for the standard permanent banner. This is the best way to slowly build Resonance for A-Rank characters like Mint, Daffodil, and Edgar.
*   **Solid Dice:** Used for limited-time S-Rank banners. Saving these is essential for hitting R1 or R2 on premium characters like Baicang or [Sakiri](/characters/sakiri-nte-free-5-star-guide).

If luck isn't on your side, the **Warp Exchange** serves as a pity system. Every time you pull a duplicate, you receive Warp Pieces. You can spend these pieces in the shop to buy Mind Shards for characters currently in the rotation. S-Rank shards are significantly more expensive and are often limited to undisclosed per month.

### City Tycoon: The Path to Free Resonance

One of the unique features of Neverness to Everness is the City Tycoon mode. This is an economic simulation where you manage businesses and properties in Hethereau. By progressing through the City Tycoon ranks, you can earn "Free Resonance" for specific characters.

For example, reaching **City Tycoon Level 18** provides a guaranteed Mind Shard for **Chiz**. This allows F2P players to eventually reach R1 or R2 for certain characters without spending a single Die in the gacha. It is highly recommended to prioritize your urban business management alongside your combat missions to ensure you don't miss these milestones.

### How Resonance Level Changes Combat and the Esper Cycle

The "Esper Cycle" is the flow of combat in NTE, involving character swaps and elemental interactions. Increasing Resonance Level directly impacts this cycle.

*   **Cooldown Management:** Many R1 passives reduce the cooldown of the character’s "Anomaly Skill" when they trigger a specific reaction. This allows for a much faster rotation.
*   **Energy Economy:** Higher Resonance nodes often provide flat energy restoration. This reduces the need for Energy Regeneration stats on your Cassettes, allowing you to focus purely on damage-oriented sub-stats.
*   **Team Synergy:** R2 bonuses are frequently team-wide. For instance, an R2 Sakiri might provide a undisclosed DMG boost to all teammates after using her ultimate, making her a top-tier support even if she isn't the primary damage dealer.

### Resource Management: Dice, Warp Pieces, and Analith

To efficiently increase Resonance, you must manage three distinct resource pools:

1.  **Dice Strategy:** Do not spread your Solid Dice across every banner. If you want a character to be viable in late-game "Ethereal Rifts," you should aim for at least R1. This usually requires pulling the character four times (one initial copy + three duplicates for A1, A2, and A3).
2.  **Warp Piece Conservation:** Save your Warp Pieces for the final shard needed to hit an R1 or R2 milestone. Spending them on A1 or A2 is often a waste if you plan to keep pulling on that character's banner.
3.  **Analith Farming:** While Mind Shards are the gatekeeper, Analith is the fuel. You can farm Analith by completing "Urban Anomalies" found throughout the city map. Ensure you have enough Analith stockpiled before a banner launch so you can instantly upgrade your new character.

## Related

![Related](/img/nte-how-to-increase-resonance-level/inline-4.webp)


Increasing your Resonance Level is only one part of the character progression puzzle in Neverness to Everness. To truly maximize your power, you must understand how Resonance interacts with other systems.

### Hunter Level and Appraisal Level Gating

While you can technically acquire Mind Shards at any time, your ability to utilize them is often gated by your **Hunter Level**. You cannot activate higher-tier Awakening nodes (specifically A4-A6) until the character has reached a certain Ascension tier. Ascension requires specific world boss drops, which are only available once your Hunter Level is high enough to increase your **Appraisal Level**.

If you find yourself unable to click the "Awaken" button despite having the shards, check your character's current level cap. You may need to complete an Appraisal Rank trial to unlock the next stage of progression.

### Best Characters to Max Resonance First

For most players, focusing on A-Rank characters for R2 is more realistic than chasing R2 on an S-Rank. 

*   **Mint (A-Rank):** Her R2 provides a significant boost to undisclosed and makes her one of the most reliable healers in the game.
*   **Edgar (A-Rank):** Reaching R1 with Edgar allows for much better crowd control during urban skirmishes.
*   **Baicang (S-Rank):** If you are a spender or have saved Solid Dice, Baicang's R1 is considered a "meta-defining" upgrade, as it significantly alters his undisclosed scaling.
*   **Nanally (S-Rank):** Her banner, "The Ichi-daime," is a frequent target for players looking to hit R1, which enhances her ability to shred enemy elemental shields.

| Character | Recommended Resonance | Priority Reason |
| :--- | :--- | :--- |
| **Baicang** | R1 | Massive jump in personal DPS and skill uptime. |
| **Chiz** | R2 | Easily obtainable through City Tycoon Level 18+. |
| **Daffodil** | R1 | Unlocks essential utility for elemental reaction teams. |
| **Sakiri** | R1 or R2 | Provides some of the best off-field buffs in the version 1.0 meta. |

### Synergizing Resonance with Arcs and Cassettes

Finally, Resonance Level should dictate how you build your character's gear.

*   **Arcs (Weapons):** If a character's R1 provides a large amount of Crit Rate, you can swap their Arc to one that focuses on Crit DMG or Elemental Mastery.
*   **Cassettes and Modules:** These gear sets often have 2-piece and 4-piece bonuses. Some Resonance nodes specifically buff the stats provided by certain Cassette sets. For example, if a character gains a flat ATK bonus at A4, using a Cassette set that provides a percentage-based ATK increase will yield much higher returns.
*   **Witch's House and Oracle Stones:** Don't forget to visit the Witch's House to use Oracle Stones. These provide temporary exploration and stamina buffs that can help you farm the materials (like Analith and Fons) needed to finalize your Awakening nodes.

By balancing your gacha pulls, participating in the City Tycoon economy, and managing your Hunter Level, you can systematically increase the Resonance Level of your roster. This progression is the key to mastering the supernatural challenges of Hethereau and ensuring your team remains competitive in the ever-evolving world of Neverness to Everness.

## Sources

- Neverness to Everness Official Website — https://nte.wanmei.com/
- Game8 NTE Character Progression Guide — https://game8.co/games/Neverness-to-Everness
- IGN Neverness to Everness Wiki — https://www.ign.com/wikis/neverness-to-everness/
- Prydwen Institute: NTE Awakening and Resonance
- Reddit r/NTE_Official: City Tycoon Rewards and Milestones — https://www.reddit.com/r/NTE_Official/comments/city-tycoon-guide/